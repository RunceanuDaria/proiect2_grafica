...
// pentru desenare cilindru 
float const U_MIN2 = -PI, U_MAX2 = PI, V_MIN2=-16,  V_MAX2 = -32 * PI;
...
void CreateVAO2(void)
{
	...
	for (int merid = 0; merid < NR_MERID; merid++)
	{
		for (int parr = 0; parr < NR_PARR + 1; parr++)
		{
			...
			// cilindru 
			float x_vf = radius2 * cosf(u); // coordonatele varfului corespunzator lui (u,v)
			float y_vf = radius2 * sin(u);
			float z_vf = v;
...
{
	glClearColor(0.0f, 0.0f, 2.55f, 0.7f);
	CreateVAO1();
	CreateVAO2();
	CreateShaders();
...
void setMVP(int x, int y, int z)
{
	...
	glm::vec3 PctRef = glm::vec3(Refx + x, Refy + y, Refz + z); // pozitia punctului de referinta
	glm::vec3 Vert = glm::vec3(Vx, Vy, Vz); // verticala din planul de vizualizare 
	view = glm::lookAt(Obs, PctRef, Vert);
	glUniformMatrix4fv(viewLocation, 1, GL_FALSE, &view[0][0]);

	// matricea de proiectie 
	projection = glm::infinitePerspective(fov, (GLfloat(width) / GLfloat(height)) + 2, znear + 1);
